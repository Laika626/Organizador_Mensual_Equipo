# Organizador mensual del equipo de Mexicanes – Gestión de sedes, voluntariado, resposables de esteriloizaciones y nómina. 

Este proyecto implementa un sistema en C++ para la asociación sin fines de lucro Mexicanes, dedicada a la esterilización de perros callejeros.  
El programa permite administrar sedes mensuales, integrantes (empleados, voluntarios y responsables), horas de trabajo y salarios, además de calcular el salario neto después de impuestos.

---

## Objetivos

- Registrar y actualizar mes con mes:
  - Sedes (nombre, municipio, responsable, capacidad).
  - Integrantes (empleados, voluntarios, responsables).
  - Horas trabajadas** y salarios.
- Calcular el salario neto después de impuestos.
- Organizar al equipo mensualmente.

---

## Clases principales

- Empleado
- Voluntario
- ResponsableEsterilizacion
